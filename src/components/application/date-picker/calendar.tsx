...
export const Calendar = ({ highlightedDates, className, 'aria-label': ariaLabel, ...props }: CalendarProps) => {
    ...
    return (
        <ContextWrapper>
            <AriaCalendar aria-label={ariaLabel ?? 'Calendar'} {...props} className={(state) => cx("flex flex-col gap-3", typeof className === "function" ? className(state) : className)}>
